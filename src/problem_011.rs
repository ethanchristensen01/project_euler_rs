...
impl<const W: usize, const H: usize, T>  Grid<W, H, T> {
  ...
  pub fn adjacency_iter<const N: usize>(&self) -> AdjacencyIterator<'_, N, W, H, T> {
    AdjacencyIterator { row: 0, col: 0, direction: Direction::default(), grid: self, is_new: true, is_done: false }
  }
}
...
impl <'a, const N: usize, const W: usize, const H: usize, T> From<&'a Grid<W, H, T>> for AdjacencyIterator<'a, N, W, H, T> {
    fn from(value: &'a Grid<W, H, T>) -> Self {
        Self { row: 0, col: 0, direction: Direction::default(), grid: value, is_new: true, is_done: false }
    }
}

impl <'a, const N: usize, const W: usize, const H: usize, T: Debug> AdjacencyIterator<'a, N, W, H, T> {
  ...
  const fn curr_adj_valid (&self) -> bool {
    match self.direction {
      _ if self.row >= H || self.col >= W
        => false,
      Direction::Right if self.col <= W - N
        => true,
      Direction::DownRight if self.row <= H - N && self.col <= W - N
        => true,
      Direction::Down if self.row <= H - N
        => true,
...
impl<'a, const N: usize, const W: usize, const H: usize, T: Debug> Iterator for AdjacencyIterator<'a, N, W, H, T> {
    type Item = [&'a T; N];
